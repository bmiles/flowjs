  ...
  device.serialPort.flush(function(error) {
    device.serialPort.on('data', function(data) {
      ...
      if (content.state === 0x00) {
        ...
        callback(new Error(stateHandler.errorHandler(content.state)));
      }
    });
  });
...
SLI1000.prototype.sensorInfo = q.function(callback); {
  var device = this;
  device.serialPort.on('open', function () {
    console.log('Serial Port Open');
  })
  .then(device.serialPort.flush)
  .then(device.serialPort.on('data')
  .then(function(data) {
    console.log(data);
    var content = readResponse(data);
    //console.log(content);
  ...
  device.serialPort.on('open', function () {
    ...
    device.serialPort.on('data', function(data) {
      ...
      if (content.state === 0x00) {
        ...
        return console.log(stateHandler.errorHandler(content.state));
      }
    });
